...
public class ReservationController : Controller
{
    ...
    public async Task<IActionResult> HotelBookingJson(BookingSearch model)
    {
        List<Hotel> hotelList = null;
        if (model != null)
        {
            hotelList = await _context.Hotels
                .Where(x => x.PricePerNight >= model.MinPrice && x.PricePerNight <= model.MaxPrice)
                .ToListAsync();

            // Retrieve booked hotels during the specified date range
            var bookedHotelIds = await _context.HotelBookings
                .Where(x => model.FromDate <= x.ToDate && model.ToDate >= x.FromDate)
                .Select(x => x.HotelId)
                .ToListAsync();
            if (bookedHotelIds.Any())
            {
                hotelList = hotelList.Where(x => !bookedHotelIds.Contains(x.Id)).ToList();
            }
            else
            {
                hotelList = hotelList;
    ...
    public async Task<IActionResult> FlightBookingJson(FlightSearch model)
    {
        List<Flight> flightList = null;
        if (model != null)
        {
            flightList = await _context.Flights
                .Where(x => x.DepartureCity == model.Departure && x.ArrivalCity == model.Arrival)
                .ToListAsync();

            // Retrieve booked hotels during the specified date range
            var bookedHotelIds = await _context.FlightBooking
                .Where(x => model.Date == x.Date)
                .Select(x => x.FlightId)
                .ToListAsync();
            if (bookedHotelIds.Any())
            {
                flightList = flightList.Where(x => !bookedHotelIds.Contains(x.Id)).ToList();
            }
            else
            {
    ...
    public async Task<IActionResult> CarBookingJson(CarRentalSearch model)
    {
        List<CarRental> carList = null;
        if (model != null)
        {
            var flightList = await _context.CarRentals
            ...
            if (bookedHotelIds.Any())
            {
                carList = flightList.Where(x => !bookedHotelIds.Contains(x.Id)).ToList();
            }
            else
            {
                carList = flightList;
